    ...
    <div className="bg-[#e8eff3]">
      ...
      <section id="home" className="max-w-6xl mx-auto px-6 pt-10">
        ...
        <div className="mt-8 grid grid-cols-12 gap-5">
          ...
          <div className="col-span-12 md:col-span-8 grid grid-cols-12 gap-5">
            ...
            <a
              href="https://www.facebook.com/AIMEESBulSU/posts/pfbid0QBMLNdvqEusEnLRtnMp6d5jxH2NGUHKFcPr7BGNu2ii6W3wUGATKcNa4po2ScTZQl"
              target="_blank"
              rel="noopener noreferrer"
            ...
            <a
              href="https://www.facebook.com/AIMEESBulSU/posts/pfbid02ijHers3CGTgvyKqmZkGb1X7XDtdX6uhnTX3J8hCWgEYbuXzUhY2eGnktoXpjJUaHl"
              target="_blank"
              rel="noopener noreferrer"
              className="block col-span-12 md:col-span-6">
              <div className="group h-[350px] rounded-2xl overflow-hidden bg-gray-200 transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-2xl cursor-pointer">
      ...
      <section id="about" className="max-w-6xl mx-auto px-6 pt-10">
        <div className="mt-12 w-full">
          <SectionTitle title={dept.programOverview.heading} />
        ...
        <div className="mt-10 grid grid-cols-12 gap-8 items-start">
          <div className="col-span-12 md:col-span-6 overflow-hidden">
            <div className="w-[80%] md:w-[90%] rounded-2xl overflow-hidden flex items-center justify-center">
              <img src={dept.images.peo} alt="Mechatronics Gears" />
            </div>
          </div>
          ...
          <div className="col-span-12 md:col-span-6 min-w-0">
            <div className="space-y-6">
              {dept.peo.bullets.map((b, idx) => (
                <div key={idx}>
                  <h4 className="text-lg font-semibold text-gray-800">PEO {idx + 1}</h4>
                  <p className="mt-2 text-gray-600 leading-relaxed text-justify break-words">{b}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="so" className="mt-10 max-w-6xl mx-auto px-6 pt-16">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
          {dept.so.outcomes.map((o, idx) => (
            <OutcomeCard key={idx} title={o.title} text={o.text} icon={outcomeIcons[idx]} />
          ))}
        </div>
      </section>
      ...
      <section id="curriculum" className="mt-10 max-w-6xl mx-auto px-6 pt-16">
        <div className="grid grid-cols-12 gap-8 items-stretch">
          <div className="col-span-12 md:col-span-6">
            ...
            {dept.curriculum.subtitle && (
              <p className="mt-2 text-sm font-medium text-gray-700">{dept.curriculum.subtitle}</p>
            )}
            <p className="mt-6 text-lg text-gray-700 leading-relaxed text-justify">{dept.curriculum.text}</p>
            {dept.curriculum.structure && (
              <p className="mt-3 text-lg text-gray-700 leading-relaxed text-justify ">{dept.curriculum.structure}</p>
            )}
          </div>
          <div className="mt-6 col-span-12 md:col-span-6 flex">
            <div className="w-full flex items-center justify-center">
              <img src={dept.images.watermark} alt="" className="w-[80%] md:w-[80%] object-contain select-none" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
          {dept.curriculum.years.map((year, idx) => (
            <div key={idx} className="group relative rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300 rounded-2xl" style={{ backgroundColor: "#1c638b" }} />
              <div className="relative z-10">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-gray-900 transition group-hover:text-white">{year.title}</h3>
                  <span className="h-3 w-3 rounded-full bg-[#1c638b] transition group-hover:bg-white" />
                </div>
                <p className="mt-2 text-sm text-gray-400 transition group-hover:text-blue-100">Hover to view details</p>
                ...
                <div className="mt-4 max-h-0 overflow-hidden opacity-0 transition-all duration-500 group-hover:max-h-96 group-hover:opacity-100">
                  <p className="text-sm text-gray-600 transition group-hover:text-blue-100">{year.description}</p>
                  <ul className="mt-3 space-y-2 text-sm text-gray-600">
                    {year.bullets.map((b, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[#1c638b] group-hover:bg-white" />
                        <span className="transition group-hover:text-white">{b}</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section id="laboratories" className="mt-10 max-w-6xl mx-auto px-6 pt-16">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {dept.laboratories.items.map((lab, idx) => (
            <div key={idx} className="group rounded-2xl border border-gray-200 bg-white p-6 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(28,99,139,0.2)] hover:bg-[#1c638b]">
              <div className=" text-xs font-semibold tracking-widest text-gray-400 group-hover:text-white/80">LAB {idx + 1}</div>
              <h3 className="mt-2 text-base md:text-lg font-bold text-gray-900 group-hover:text-white transition">{lab}</h3>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="faculty" className="mt-10 max-w-6xl mx-auto px-6 pt-16">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {dept.faculty.members.map((member, idx) => (
            <div key={`${member.name}-${idx}`} className="group overflow-hidden rounded-[2rem] border border-gray-200 bg-white transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(28,99,139,0.22)]">
              ...
              <div className="p-5 text-center transition-colors duration-300 group-hover:bg-[#1c638b]">
                <h3 className="text-base font-bold uppercase leading-snug text-gray-900 transition-colors duration-300 group-hover:text-white">{member.name}</h3>
                <div className="mx-auto my-3 h-[2px] w-14 bg-[#1c638b]/30 transition-colors duration-300 group-hover:bg-white/50" />
                <p className="text-sm font-semibold uppercase tracking-wider text-gray-500 transition-colors duration-300 group-hover:text-white/85">{member.role}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="careers" className="mt-10 max-w-6xl mx-auto px-6 pt-16">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {dept.careers.cards.map((card, idx) => {
            ...
            return (
              <div key={idx} className="group rounded-2xl border-2 border-[#1c638b]/20 bg-[#e8eff3] p-6 text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(28,99,139,0.25)] hover:bg-[#1c638b] hover:border-[#1c638b]">
                ...
                <h3 className="mt-2 font-bold text-gray-900 group-hover:text-white transition">{card.title}</h3>
                <p className="mt-2 text-sm text-gray-700 leading-relaxed text-justify group-hover:text-white/90 transition">{card.text}</p>
              </div>
            );
          })}
        </div>
      ...
      <section id="contact" className="max-w-6xl mx-auto px-6 pt-16">
        <div className="rounded-2xl border bg-gray-50 p-6 md:p-8">
          ...
          <p className="mt-7 text-xl text-gray-900 font-bold">Engr. Elijah Intal</p>
          ...
          <div className="mt-7 flex items-center gap-3">
            ...
            <a href="mailto:arvinjulius.tullao@bulsu.edu.ph" className="text-lg text-gray-700 font-semibold hover:text-[#1c638b] transition">
...
function OutcomeCard({ 
  ...
  title: string; 
  text: string; 
  icon: ComponentType<{ className?: string }> // Use ComponentType instead of React.ElementType
}) {
  return (
    <div className="group relative rounded-2xl border border-gray-200 bg-white p-6 text-center transition-all duration-300 hover:-translate-y-2 hover:bg-[#1c638b] hover:shadow-[0_15px_30px_rgba(28,99,139,0.25)]">
      <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-[#1c638b]/10 transition group-hover:bg-white/20">
        <Icon className="h-6 w-6 text-[#1c638b] group-hover:text-white" />
      </div>
      <div className="text-lg font-bold text-gray-900 transition group-hover:text-white">{title}</div>
      <div className="mt-2 text-sm leading-relaxed text-gray-700 transition group-hover:text-white/90">{text}</div>
    </div>
  );
}
